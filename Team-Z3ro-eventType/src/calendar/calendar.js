...
function CalendarApp(date) {
  ...
  this.apts = [
    { name: `New Year's Eve`,
      day: new Date(2021, 11, 31),
      startTime: new Date(2021, 11, 31,18),
      endTime: new Date(2021, 11, 31, 23)},
      { name: `Project presentation CSI-5130`,
      day: new Date(2021, 10, 30),
      startTime: new Date(2021, 10, 30,17),
      endTime: new Date(2021, 10, 30, 19)},
  ];

  this.aptDates = [new Date(2021, 11, 31).toString(), new Date(2021, 10, 30).toString()];
  this.eles = {
  };
...
//This is where list of created events shows up in day view
CalendarApp.prototype.showEventsCreateElesView = function(events) {
  ...
  events.forEach(function(event){
    ...
    //prevent from overwriting last object?
    

    //this is where the event output shows up in the calendar view
    html += "<span class='event-name'>" + event.name + "</span>";
    //this creates an attribute for the event 
    //html += "<span class='eventType'>" + document.getElementById("eventType").value + "</span>";
    html += "<span class='eventType'>" + event.type + "</span>";
...
CalendarApp.prototype.addEvent = function() {
  ...
  var type = this.dayEventAddForm.eventType.value.trim();
  ...
  this.apts.push({
    ...
    type: type
    //String('Educational','Exercise','Family','Fun','Social','Work','Other')
  });
  this.closeNewEventBox();
...
CalendarApp.prototype.validateAddEventInput = function() {
  ...
  //checks to see if eventType has a type indicated 
  if (!type || type == null){
    _errors = true;
    this.dayEventAddForm.eventType.classList.add("add-event-edit--error");
    this.dayEventAddForm.eventType.focus();
  } else {
    this.dayEventAddForm.eventType.classList.remove("add-event-edit--error");
...
CalendarApp.prototype.resetAddEventBox = function(){
  ...
  //addition 12/4 but it causes it to dissapear 
  //this.dayEventAddForm.eventType.value = '';
  //this.dayEventAddForm.eventType.classList.remove("add-event-edit--error");
